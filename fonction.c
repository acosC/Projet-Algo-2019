...
void Premiere_version(Position * paquet,const char* nom_fichier)
{
    ...
    int temps_ecoule;
    int* tab_position = malloc((LONGUEUR_CABLE/PRECISION) * sizeof(int));//utilisation d'une allocation dynamique pour gérer une grande quantité de valeur
    ...
    for(int position = 0; position < (LONGUEUR_CABLE/PRECISION); position++)//on parcours le tableau créé au dessus
    {
        somme = 0;

        if (position > (LONGUEUR_CABLE/PRECISION) - DIST_MAX_VOISIN - 1)
            max = (LONGUEUR_CABLE/PRECISION) - 1;// car les case vont de 0 à long LONGUEUR_CABLE-1 soit LONGUEUR_CABLE case
        else
...
void Deuxieme_version(Position * paquet,const char* nom_fichier)
{
    ...
    for(int i =0; i < LONGUEUR_CABLE; i++) // On parcourt une première fois le tableau
    {
        ...
        if (i > (LONGUEUR_CABLE - DIST_MAX_VOISIN - 1))
            max = LONGUEUR_CABLE ;  // car les cases vont de 0 à LONGUEUR_CABLE-1 soit LONGUEUR_CABLE case
        else
            max = i + DIST_MAX_VOISIN;

        if ( i + 1 < DIST_MAX_VOISIN)   //pour éviter que le min soit au en dessous de 0 car impossible
            min = 0;
        else
        ...
        for (int j =0; j < NOMBRE_DEFORMATIONS; j++) // On parcourt le paquet
        {
    ...
    temps_ecoule = relever_chronometre_ms(); //on recupere le temps écoulé pour cette version
...
void Troisieme_version(Position * paquet, const char* nom_fichier)
{
    ...
    int temps_ecoule;
    register int * tab_position = malloc((LONGUEUR_CABLE/PRECISION) * sizeof(int));//utilisation d'une allocation dynamique pour gérer une grande quantité de valeur

    /* création d'un tableau de transfère */
    /* tab_position est tableau des déformation en fonction des positions */
    /* pour chaque position on a le nombre de déformation en cette position */
    /* optimiser par rapport à la PREMIERE VERSION */
    for (int position = 0; position < (LONGUEUR_CABLE/PRECISION); position++) //on initilaise à 0 par sécurité
        tab_position[position] = 0;
